---
title: Ingestion
description: How documents get in and become searchable — formats, OCR, cleaning, chunking, upload, and the arXiv corpus fetcher.
sidebar_position: 4
---
...
flowchart TD
    ...
    EMBED --> CHROMA[("Chroma: engineering_kb")]
```
...
> vector. The separate **lexical** tokenizer (`_tokenize`, keeps `40%`/`12.4` whole) is a
...
```bash
curl -X POST "localhost:8000/v1/upload?filename=term_sheet.pdf" \
  --data-binary @term_sheet.pdf
```
...
The bundled `data/` is synthetic. To ingest **real** documents, fetch open-access papers
from the **arXiv API** — abstracts in the Software Engineering (`cs.SE`) and Distributed
/ Parallel Computing (`cs.DC`) categories are open access and freely redistributable
(unlike copyrighted books, deliberately excluded). The script saves each paper's title +
abstract:
...
python scripts/fetch_corpus.py --dry-run       # list URLs, download nothing
python -m app.ingest                           # then (re)build the index
```

Design notes: **stdlib only** (`urllib`, `xml.etree`, no new dependency); **idempotent**
(skips files already present); **polite** (sends a descriptive User-Agent, rate-limited to
>=3s between requests per arXiv guidance); **offline-safe** (network/parse errors are logged
and skipped, never fatal); **provenance** (every download is recorded in `data/SOURCES.md`
with its URL). Default queries: `cat:cs.SE`, `cat:cs.DC`, `cat:cs.SE+OR+cat:cs.DC`. Set a
contact (a GitHub URL is fine):

```bash
ARXIV_USER_AGENT="my-project (github.com/you)" python scripts/fetch_corpus.py
```

The fetcher parses the Atom feed itself (title, summary, published date, id) and writes
`data/corpus/<arxiv_id>.txt`, which then flows through the normal ingest pipeline as a
text file.
