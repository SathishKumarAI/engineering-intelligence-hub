...
def test_ask_returns_answer_with_citations(client):
    r = client.post("/v1/ask", json={"question": "How do I roll back a deploy?"})
    assert r.status_code == 200
    body = r.json()
    assert body["answer"]
...
def test_metrics_endpoint_exposes_prometheus(client):
    client.post("/v1/ask", json={"question": "rate limit?"})
...
def test_auth_required_when_api_key_set(monkeypatch, fake_engine):
    ...
    main.app.dependency_overrides[main.get_engine] = lambda: fake_engine
    with TestClient(main.app) as c:
        assert c.post("/v1/ask", json={"question": "rate limit?"}).status_code == 401
        ok = c.post("/v1/ask", json={"question": "rate limit?"}, headers={"X-API-Key": "secret"})
